...
router.post('/edit-action', upload.single('upload'), (req, res)=>{
    ...
    if(req.file == undefined){
        ...
        conn.query('SELECT file FROM tb_pokemon WHERE id=?',[id],(error, result)=>{
            ...
            conn.query('DELETE file FROM tb_pokemon WHERE id=?',[id],(error, result)=>{

               fs.unlink('upload/'+oldImage,(err)=>{
                   if(err){
                       console.log(err)
                   }else{
                       //conn.query('UPDATE tb_pokemon SET? WHERE id = ?',[{file: file,name: name, hp: hp, attack: attack,defense: defense, speed: speed,special_attack: special_attack,special_defense:special_defense}, id],(error, result)=>{
                        conn.query('UPDATE tb_pokemon SET? WHERE id = ?',[{file: file,name: name, atributo: atributo}, id],(error, result)=>{ 
                       if(error){
                            throw error
                       }else{
                            res.redirect('/')
